...
const TopArtistsTable = ({ artists = [] }) => {
  return (
    <div className="rounded-2xl border border-white/10 bg-zinc-950 p-5">
      <h3 className="mb-5 text-lg font-semibold text-white">Top Artists</h3>
        ...
        <table className="w-full min-w-[650px] text-left text-sm">
          <thead>
            <tr className="border-b border-white/10 text-xs uppercase text-zinc-500">
              ...
              <th className="pb-3 font-medium">Songs</th>
            ...
            {artists.length > 0 ? (
              artists.map((artist) => (
                <tr key={artist._id} className="border-b border-white/5">
                  ...
                  <td className="py-4 text-zinc-300">{artist.songs || 0}</td>
                  <td className="py-4 text-zinc-300">
                    {artist.followers || 0}
                  </td>
                  <td className="py-4 text-zinc-300">{artist.streams || 0}</td>
                  <td className="py-4 text-zinc-300">₹{artist.revenue || 0}</td>
                </tr>
              ))
            ) : (
              <tr>
